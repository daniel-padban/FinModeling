...
class LSTMTrainer():
    def __init__(self, model:nn.Module,train_dataloader:DataLoader, test_dataloader:DataLoader, config_dict:dict, scaler_dict:dict,report_freq:int,device):
        self.model = model
    ...
    def MSPELoss(self,pred:torch.Tensor, target:torch.Tensor):
        ...
        MSPE = SPE.mean()
        return MSPE
    def inverse_standard_scaling(self,input:torch.Tensor,std:ndarray,mean:ndarray,):
        x = std * input + mean
        return x
    ...
    def _train_loop(self):
        ...
        for i, (X, y, tickers) in enumerate(self.train_dataloader):
            ...
            for i in range(tickers):
                scaler:StandardScaler = self.scaler_dict[tickers[i]]
                current_pred = pred.select(0,i)
                scaler_std = scaler.scale_
                scaler_mean = scaler.mean_
                descaled_pred = self.inverse_standard_scaling(current_pred,scaler_std,scaler_mean)
                descaled_preds_concat_list.append(descaled_pred)
            joined_descaled_preds = torch.cat(descaled_preds_concat_list,dim=0)
